...
const getBestSchoolPrice = (school: SchoolDetails, courseType: string) => {
  ...
  if (courseType === "ingles-visa-de-trabajo" && raw) {
    const offer = raw.oferta ? parseFloat((raw.oferta as string).replace(/[^0-9.,]/g, "").replace(",", ".")) : null;
    const regular = raw.precio ? parseFloat((raw.precio as string).replace(/[^0-9.,]/g, "").replace(",", ".")) : 0;

    return {
      price: !isNaN(regular) && regular > 0 ? regular : 0,
      offer: !isNaN(offer ?? NaN) && offer && offer > 0 ? offer : null,
      fromLabel: false,
    };
  }

  if (typeof school.bestOffer === "number" && school.bestOffer > 0) {
    return { price: school.bestPrice ?? 0, offer: school.bestOffer, fromLabel: false };
  }

  if (typeof school.bestPrice === "number" && school.bestPrice > 0) {
...
function SchoolCard({ school, viewType, courseType }: SchoolCardProps) {
  ...
  const antiguedad = school.description?.añoFundacion ? new Date().getFullYear() - school.description.añoFundacion : null;
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-1 flex items-center">
              ...
              <span className="ml-2 text-sm text-gray-600">{parseFloat(String(rating)).toFixed(1)}</span>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col items-center sm:items-end">
              {offer ? (
                ...
                <div className="text-2xl font-bold text-gray-800">
                  {fromLabel ? (
                    <>Desde <span className="text-blue-600 text-3xl font-bold">€{price.toLocaleString()}</span></>
                  ) : (
